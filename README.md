...
The diagram below outlines the ingestion process when a PDF is uploaded:
...
sequenceDiagram
    ...
    rect rgb(240, 248, 255)
        ...
        Celery->>Celery: Stage 1: Extract text using PDFExtractor (Page-by-page)
        Celery->>Celery: Stage 2: Chunk text with StructureAwareChunker (Max & Overlap limit)
        Celery->>Celery: Stage 3: Prepare documents with metadata
        Celery->>Weaviate: Stage 4: Store & vectorise chunks
    end
...
Asynchronously uploads and schedules a PDF for ingestion.
...
Fetches the progress of an active or historical ingestion task.
...
Queries Weaviate for similarity match across all ingested text chunks.
